 ...
 *    Date(2015,1,1)    => false
 */
const isLeapYear = (date) => ((y) => ((y % 100) && !(y % 4)) || !(y % 400))(date.getFullYear());
...
function timeSpanToString(startDate, endDate) {
  ...
  return format.match(/{{.+?:\d}}/g).reduce((result, replaceStr) => {
    const [getUTCfunc, padAmount] = replaceStr.slice(2, -2).split(':');
...
function angleBetweenClockHands(date) {
  ...
  const anglePIs = Math.abs((d.getUTCHours() % 12) * 30 - d.getUTCMinutes() * 5.5) / 180;
  return (anglePIs > 1 ? anglePIs - 1 : anglePIs) * Math.PI;
}
